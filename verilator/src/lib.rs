...
impl ModuleGenerator {
    ...
    fn _generate_files(&mut self, krate: &Path) {
        ...
        let rs_file = out_dir.join("verilator.rs");
        let rs_out = BufWriter::new(t!(File::create(&rs_file)));

        let cpp_file = rs_file.with_extension("cpp");
        let cpp_out = BufWriter::new(t!(File::create(&cpp_file)));
...
extern "C" {{
  ...
  void
  {c_ty}_delete({c_ty}* __ptr) {{
    delete __ptr;
  }}

  // API METHODS
  void
  {c_ty}_eval({c_ty}* __ptr) {{
    __ptr->eval();
  }}

  void
  {c_ty}_final({c_ty}* __ptr) {{
    __ptr->final();
  }}
"#,
            c_ty = c_ty
        ...
        if let Some(ref clock) = ports.clock {
            ...
            t!(writeln!(
                self.cpp,
                r#"  void
  {c_ty}_{clk}({c_ty}* __ptr, vluint8_t __v) {{
    __ptr->{clk} = __v;
  }}
"#,
                c_ty = c_ty,
        ...
        if let Some(ref reset) = ports.reset {
            ...
            t!(writeln!(
                self.cpp,
                r#"  void
  {c_ty}_{rst}({c_ty}* __ptr, vluint8_t __v) {{
    __ptr->{rst} = __v;
  }}
"#,
                c_ty = c_ty,
        ...
        for input in &ports.inputs {
            ...
            t!(writeln!(
                self.cpp,
                r#"  void
  {c_ty}_set_{input}({c_ty}* __ptr, {v_ty} __v) {{
    __ptr->{input} = __v;
  }}
"#,
                c_ty = c_ty,
...
"#,
        ...
        pub fn {c_ty}_get_{inout}({c_ty}: *mut {c_ty}) -> {ffi_ty};"#,
            ...
            t!(writeln!(
                self.cpp,
                r#"  void
  {c_ty}_set_{inout}({c_ty}* __ptr, {v_ty} __v) {{
    __ptr->{inout} = __v;
  }}
 
  {v_ty}
  {c_ty}_{inout}({c_ty}* __ptr) {{
    return __ptr->{inout};
  }}
"#,
                c_ty = c_ty,
